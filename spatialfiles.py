...
def rasterImport(file):
	...
	#
	driverShort = raster.GetDriver().ShortName
	driver = gdal.GetDriverByName(driverShort)
	...
	sumValues = data[np.where(np.logical_not(data == nandat))]
	sumValue = sumValues.sum()
	numberValues = len(sumValues)
	averageValue = sumValue/numberValues
	volumeOfValues = sumValue*(fabs((transf[1]*transf[5])))
	meanOfValues = np.mean(sumValues)
	medianOfValues = np.median(sumValues)
	stdDevValues = np.std(sumValues)
	skewValues = stats.skew(sumValues)
	kurtosisValues = stats.kurtosis(sumValues)
	print file
	print "Min: {0:0.3f} Mean: {1:0.3f} StdDev: {2:0.3f} Max: {3:0.3f} Median: {4:0.3f}\n".format(mindat, meanOfValues, stdDevValues, maxdat, medianOfValues)
	print "Mean: {0:0.3f} StdDev {1:0.3f} Skewness: {2:0.3f} Kurtosis{3:0.3f}\n".format(meanOfValues, stdDevValues, skewValues, kurtosisValues)
	meta = {'transform':transf,'projection':projec,'corners':corners,'dimension':dims,'dats':dats,'sumValue':sumValue,'numberValues':numberValues,'averageValue':averageValue,'volumeOfValues':volumeOfValues}
	if srs.IsProjected:
	...
	print 'Sum of pixel values = ',sumValue,' in range ',mindat,' to ',maxdat
	print 'From ',numberValues,' of ',rows,' X ',cols,' = ',rows*cols,' pixels, or ',((1.0*numberValues)/(rows*cols))*100,'%'
	print 'Average then is ',averageValue
	print 'Pixels size, x:',transf[1],' y:',transf[5]
	print '"Volume" is then the sum * 1 pixel area: ',volumeOfValues
...
def rasterExport(outData, meta, filnm):
	...
	dataOut = datDriver.Create(filnm, inCols, inRows, 1, gdal.GDT_Float32)
	dataOut.SetGeoTransform(meta['transform'])
	dataOut.SetProjection(meta['projection'])
	outData_m = np.flipud(outData)
	outData_m = np.where(np.isnan(outData_m),meta['dats'][0],outData_m)
	dataOut.GetRasterBand(1).WriteArray(outData_m)
	dataOut.GetRasterBand(1).SetNoDataValue(meta['dats'][0])
	dataOut.GetRasterBand(1).ComputeStatistics(True)
	dataOut = None
	return 0
#
def demslicer(file,outfolder,slice=10):
	"""Slice a raster (usually a DEM) into bands according to pixel value
	To use: demslicer(file, outfolder, slice)
	slice defaults to 10"""
	print slice
	print '\nImporting ',file
...
def xydiff(atype, bearing, distance):
	...
	To use: ediff, ndiff = xydiff(atype, bearing, distance)
	atype is the angle type, degrees (d), gon (g) or radians (r)"""
...
def hordist(atype,vangle,slpdist):
	...
	To use: hdist, vdist = hordist(atype,vangle,slpdist)
	atype is the angle type, degrees (d), gon (g) or radians (r)"""
...
def pythag2p(Xa,Ya,Xb,Yb):
	...
	To use: hypo = pythag2p(Xa,Ya,Xb,Yb)"""
...
def extender(ax,ay,bx,by,length):
	"""Extend a line between two points by a set distance
	To use: cx,cy = extender(ax,ay,bx,by,length)"""
...
#
def pt2fmt(pt):
	"""Called by GetRasterVals. Needs no input from user"""
	fmttypes = {
	'Byte': 'B',
	'Int16': 'h',
	'UInt16': 'H',
...
#	 ypix = ydifpix -1
	...
	for i in range(1,bandcount+1):
		...
		dataTypeString = gdal.GetDataTypeName(band.DataType)
		try:
			fmt = pt2fmt(bandtype)
		except:
			print "Could'nt get data type"
...
def makevrt(fileName, epsg, Xcol, Ycol, Zcol = 'none'):
	"""Called by vrtMaker. Needs no user input"""
	...
	rmName = outName.replace(" ","\ ")
	if  os.path.exists(outName):
		os.system('rm {}'.format(rmName))
	...
	return outName
#
def idw(dataFile, DEM, power=2.0, smoothing=2.0, **kwargs):
	"""Implement gdalgrid for IDW"""
	# Set output directory
	namestr, ext_,outDir, full_ = namer(dataFile)
	srcDir = outDir.replace(" ","\ ")
	outDir = os.path.join(outDir, namestr)
	if not os.path.exists(outDir):
		os.makedirs(outDir)
	# Make vrt for extrapolation
	if 'epsg' in kwargs:
		Zfield, namevrt, epsg = vrtMaker(dataFile, kwargs['epsg'])
	else:
	...
	outsider = meta['dimension']['rows']
	outsidec = meta['dimension']['cols']
	# Create gdal command for IDW
	layer = namestr
	outDir = outDir.replace(" ","\ ")
	namevrt = namevrt.replace(" ","\ ")
	idwfile = os.path.join(outDir, namestr + '_' + Zfield + '_idw.tif')
	gridout = "cd {0}; gdal_grid -a invdist:power={1:0.2f}:smoothing={2:0.2f} -txe {3:0.0f} {4:0.0f} -tye {5:0.0f} {6:0.0f} -outsize {7:0.0f} {8:0.0f} -a_srs EPSG:{9} -of GTiff -ot Float64 -l {10} {11} {12}".format(srcDir, power, smoothing, txel, txeu, tyel, tyeu, outsider, outsidec, epsg, layer, namevrt, idwfile)
	print gridout
	os.system(gridout)
	return idwfile
#
